...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    cap("resources.home", "协同首页", "首页", "资源、需求和匹配概览", "resources", "resources", "/resources", "/api/v1/resources", "handshake", order=51),
    cap("resources.leads", "线索", "线索", "业务线索和联系意向", "resources", "resources", "/network?tab=intents", "/api/v1/network/contact-intents", "target", order=52),
    cap("resources.opportunities", "合作机会", "机会", "合作机会管理", "resources", "resources", "/opportunities", "/api/v1/opportunities", "handshake", order=53),
    cap("resources.tasks", "任务与跟进", "任务", "协作任务和商务跟进", "resources", "resources", "/workspace?tab=tasks", "/api/v1/opportunities", "list-checks", order=54),
    cap("resources.meetings", "会议与材料", "会议", "项目时间线和会议材料", "resources", "resources", "/opportunities?tab=timeline", "/api/v1/opportunities", "clock", order=55),
    cap("resources.supply", "资源供给", "供给", "供给资源", "resources", "resources", "/resources?direction=supply", "/api/v1/resources", "package-plus", order=56),
    cap("resources.demand", "资源需求", "需求", "需求资源", "resources", "resources", "/resources?direction=demand", "/api/v1/resources", "package-search", order=57),
    cap("resources.match", "资源匹配", "匹配", "供需匹配", "resources", "resources", "/resources?tab=match", "/api/v1/resources", "git-compare", order=58),
    cap("resources.followups", "商务跟进", "跟进", "商务跟进记录", "resources", "resources", "/opportunities?tab=followups", "/api/v1/opportunities", "message-square", order=59, legacy=("/actions",)),
    ...
    cap("club.member_center", "会员", "会员", "会员身份和会员中心", "club", "club", "/member", "/api/v1/me/membership-context", "id-card", "membership.view_self", "membership", order=62),
    cap("club.events", "活动", "活动", "俱乐部活动", "club", "club", "/club/events", "/api/v1/events", "calendar", order=63),
    cap("club.matching", "供需与匹配", "匹配", "会员供给、需求和撮合", "club", "club", "/club/matches", "/api/v1/resources", "shuffle", order=64),
    cap("club.operations", "运营管理", "运营", "会员管理、活动管理和供需管理", "club", "club", "/club/operations", "/api/v1/club/operations", "landmark", "manage_club", clients=("web", "admin"), order=65),
    cap("club.resources", "会员供需", "供需", "会员供给和需求", "club", "club", "/club", "/api/v1/resources", "package", order=66),
    cap("club.applications", "会员申请", "申请", "会员申请管理", "club", "club", "/club/admin/applications", "/api/v1/me/memberships", "clipboard-check", "manage_club", clients=("web", "admin"), order=67),
    cap("club.notifications", "通知", "通知", "俱乐部通知", "club", "club", "/club?tab=notifications", "/api/v1/me/club-context", "bell", order=68),
    cap("club.members", "会员管理", "会员管理", "会员管理", "club", "club", "/club/members", "/api/v1/me/memberships", "users-round", "manage_club", clients=("web", "admin"), order=69),
    cap("club.event_admin", "活动管理", "活动管理", "活动管理", "club", "club", "/club/events", "/api/v1/events", "calendar-cog", "manage_club", clients=("web", "admin"), order=70),
    cap("club.supply_admin", "供需管理", "供需管理", "供需管理", "club", "club", "/club/resources", "/api/v1/resources", "package-check", "manage_club", clients=("web", "admin"), order=71),
    cap("club.import", "数据导入", "导入", "会员数据导入", "club", "club", "/club/import", "/api/v1/memberships", "upload", "manage_club", clients=("web", "admin"), order=72),
