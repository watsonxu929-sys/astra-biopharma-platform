...
class IntelligenceItem(Base):
    ...
    source_record_type: Mapped[str | None] = mapped_column(String(50), nullable=True, index=True)
...
class MarketResource(Base):
    ...
    owner_person_id: Mapped[int | None] = mapped_column(Integer, nullable=True)  # FK: people.id
    organization_id: Mapped[int | None] = mapped_column(Integer, nullable=True)  # owner Organization
    visibility: Mapped[str] = mapped_column(String(20), default="organization", index=True)
    legacy_source_type: Mapped[str | None] = mapped_column(String(50), nullable=True)
    legacy_source_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
